...
func TestExpressions(t *testing.T) {
	...
	expressionTests := []struct {
		...
		{
			expr: `http_requests * 3`,
			output: []string{
				`{group="canary", instance="0", job="api-server"} => 900 @[%v]`,
				`{group="canary", instance="1", job="api-server"} => 1200 @[%v]`,
				`{group="canary", instance="0", job="app-server"} => 2100 @[%v]`,
				`{group="canary", instance="1", job="app-server"} => 2400 @[%v]`,
				`{group="production", instance="0", job="api-server"} => 300 @[%v]`,
				`{group="production", instance="1", job="api-server"} => 600 @[%v]`,
				`{group="production", instance="0", job="app-server"} => 1500 @[%v]`,
				`{group="production", instance="1", job="app-server"} => 1800 @[%v]`,
			},
		},
		{
			expr: `3 * http_requests`,
			output: []string{
				`{group="canary", instance="0", job="api-server"} => 900 @[%v]`,
				`{group="canary", instance="1", job="api-server"} => 1200 @[%v]`,
				`{group="canary", instance="0", job="app-server"} => 2100 @[%v]`,
		...
		{
			expr: `http_requests / http_requests`,
			output: []string{
				`{group="canary", instance="0", job="api-server"} => 1 @[%v]`,
				`{group="canary", instance="1", job="api-server"} => 1 @[%v]`,
				`{group="canary", instance="0", job="app-server"} => 1 @[%v]`,
				`{group="canary", instance="1", job="app-server"} => 1 @[%v]`,
				`{group="production", instance="0", job="api-server"} => 1 @[%v]`,
				`{group="production", instance="1", job="api-server"} => 1 @[%v]`,
				`{group="production", instance="0", job="app-server"} => 1 @[%v]`,
				`{group="production", instance="1", job="app-server"} => 1 @[%v]`,
			},
		},
	}
