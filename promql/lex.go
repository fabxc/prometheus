...
type lexer struct {
	...
	stringOpen  rune // quote rune with which the current string was opened
}
...
func lexStatements(l *lexer) stateFn {
	...
	case r == '%':
		l.emit(itemREM)
	case r == '+':
		if strings.HasPrefix(strings.ToUpper(l.input[l.pos:]), "INF") {
			l.pos += 3
			l.emit(itemNumber)
		} else {
			l.emit(itemADD)
	...
	case r == 'N' || r == 'n' || r == 'I' || r == 'i':
		n2 := strings.ToUpper(l.input[l.pos:])
