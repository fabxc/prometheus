...
// setupQuery sets up a context for the query execution. The query timeout starts
// ticking immediately. Thus, execution is expected to start right after that.
//
...
// checkContext terminates the execution if the context is done. It's a no-op otherwise.
...
func (ng *Engine) exec(ctx context.Context, q *query) (res Result) {
...
func (ng *Engine) execEvalStmt(ctx context.Context, query *query, s *EvalStmt) Result {
