...
func (ix *indexer) Querier() (*tindex.Querier, error) {
...
func (ix *indexer) index(cds ...*chunkDesc) error {
	...
	for i, cd := range cds {
		...
		if ts <= ix.mc.highTime {
			b.SecondaryIndex(id, tindex.Term{
...
func (ix *indexer) reindexTime(ids ChunkIDs, ts model.Time) error {
	...
	sort.Sort(ids)
	t := tindex.Term{Field: timeSliceField, Val: timeString(ts)}

	for _, id := range ids {
		b.SecondaryIndex(tindex.DocID(id), t)
	}
	return b.Commit()
}
